...
# dair-ai/emotion label names (6 classes)
EMOTION_LABELS = [
...
def create_mtl_dataset():
    ...
    # dair-ai/emotion has single labels
    print("Loading Emotion Dataset (dair-ai/emotion)...")
    ...
    # dair-ai/emotion has train / validation / test splits
    def emotion_to_df(split_ds):
        df = split_ds.to_pandas()
        df = df.rename(columns={'label': 'emotion_label'})[['text', 'emotion_label']]
        return df

    emotion_splits = {
        'train':      emotion_to_df(emotion_dataset['train']),
        'validation': emotion_to_df(emotion_dataset['validation']),
        'test':       emotion_to_df(emotion_dataset['test']),
    }
    ...
    # Emotion label map (dair-ai/emotion)
    emotion_map = pd.DataFrame(
        enumerate(EMOTION_LABELS), columns=['label_id', 'label_name']
    )
    ...
    print(f"  Total emotion classes : {len(EMOTION_LABELS)} (dair-ai/emotion)")
